...
const NAV_ITEMS = [
  { label: "Dashboard", route: "/teacher", icon: Home },
  { label: "My Classes", route: "/teacher/classes", icon: Users },
  { label: "Timetable", route: "/teacher/timetable", icon: Clock },
  { label: "Take Attendance", route: "/teacher/attendance", icon: CalendarCheck },
  { label: "Assignments", route: "/teacher/assignments", icon: BookOpen },
  { label: "Submissions", route: "/teacher/submissions", icon: FileCheck },
  { label: "Exams", route: "/teacher/exams", icon: Trophy },
  { label: "PTM Meetings", route: "/teacher/ptm", icon: CalendarCheck },
  { label: "Circulars", route: "/teacher/notices", icon: Megaphone },
  { label: "Messages", route: "/teacher/messages", icon: MessageSquare },
  { label: "Notifications", route: "/teacher/notifications", icon: Bell },
  { label: "My Profile", route: "/teacher/profile", icon: User },
  { label: "Settings", route: "/teacher/settings", icon: Settings },
];

const MOBILE_BOTTOM_ITEMS = [
  { label: "Home", route: "/teacher", icon: Home },
  { label: "Classes", route: "/teacher/classes", icon: Users },
  { label: "Attendance", route: "/teacher/attendance", icon: CalendarCheck },
  { label: "Tasks", route: "/teacher/assignments", icon: BookOpen },
  { label: "More", route: "#more", icon: Menu },
];

const TeacherLayoutContent: React.FC<{ children: React.ReactNode }> = ({ children }) => {
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-xs font-black tracking-tight text-foreground line-clamp-1">
                {organization?.name || "Teacher Portal"}
              </h2>
        ...
        {teacher && (
          <div className="p-3 border-b border-border bg-surface/50 flex items-center gap-3">
            ...
            <div className="truncate">
              <p className="text-xs font-extrabold text-foreground truncate">{teacher.fullName}</p>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-xs font-extrabold text-foreground truncate max-w-[180px]">
                {teacher?.fullName || organization?.name || "Teacher Portal"}
              </h2>
              <p className="text-[9px] font-bold text-primary uppercase">
                {teacher?.designation || "Faculty Workspace"}
              </p>
            </div>
          </div>
        </header>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex flex-col bg-background/95 backdrop-blur-md">
          <div className="flex items-center justify-between p-4 border-b border-border bg-card">
            <h3 className="text-sm font-extrabold text-foreground">Teacher Navigation</h3>
            <button
