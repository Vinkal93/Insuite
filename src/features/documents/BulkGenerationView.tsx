...
export const BulkGenerationView: React.FC = () => {
  ...
  useEffect(() => {
    const loadInitial = async () => {
      ...
      try {
        const [cls, dts, tmpls] = await Promise.all([
          getSchoolClasses(organization.id),
          listDocumentTypes(organization.id),
          listDocumentTemplates(organization.id),
        ]);
        setClasses(cls);
        const studentTypes = dts.filter((d) => d.personType === "STUDENT");
        setDocTypes(studentTypes);
        setTemplates(tmpls.filter((t) => t.personType === "STUDENT"));

        if (cls.length > 0) setSelectedClassId(cls[0].id);
        if (studentTypes.length > 0) setSelectedTypeId(studentTypes[0].id);
        if (tmpls.length > 0) setSelectedTemplateId(tmpls[0].id);
      } catch (err) {
        console.error("loadBulkInitial error:", err);
      }
    };
    loadInitial();
  }, [organization]);
  ...
  const handleGenerate = async () => {
    ...
    if (
      !confirm(
        `Generate ${docType?.name || "certificates"} for ${selectedStudentIds.length} students in Class ${cls?.name || ""}?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      navigate({ to: "/documents/certificates" });
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Generate and issue certificates for an entire class roster in one operation.
          </p>
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-border bg-card p-6 shadow-soft space-y-4 text-xs">
        <h3 className="font-extrabold text-sm text-foreground">Select Class & Certificate</h3>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            <label className="block font-semibold text-foreground mb-1">Template *</label>
            <select
      ...
      <div className="rounded-3xl border border-border bg-card p-6 shadow-soft space-y-4 text-xs">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            />
            <label
              htmlFor="bulkSelectAll"
              className="font-extrabold text-foreground cursor-pointer"
            >
        ...
        <div className="pt-3 border-t border-border flex justify-end">
          ...
          >
            ...
            {isGenerating
              ...
              : `Generate ${selectedStudentIds.length} Certificates`}
          </Button>
        </div>
      </div>
    </div>
