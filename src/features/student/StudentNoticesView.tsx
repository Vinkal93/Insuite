...
export const StudentNoticesView: React.FC = () => {
  ...
  const loadNotices = async () => {
    ...
    setError(null);
    try {
      const list = await listAnnouncements(organization.id, { targetAudience: "STUDENTS" });
      setNotices(list);
    } catch (err: any) {
      console.error("loadStudentNotices error:", err);
      setError(err.message || "Failed to load student notices.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-black tracking-tight text-foreground sm:text-3xl">
          Student Notices & Bulletins
        </h1>
        <p className="text-xs text-muted-foreground mt-0.5">
          Official school announcements, student activities, and holiday circulars.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-3xl border border-border bg-card p-12 text-center shadow-soft">
          ...
          <p className="mt-1 text-xs text-muted-foreground">All recent school bulletins have been reviewed.</p>
        </div>
      ) : (
