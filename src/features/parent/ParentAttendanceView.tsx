...
export const ParentAttendanceView: React.FC = () => {
  ...
  const loadAttendance = async () => {
    ...
    try {
      ...
      const [sum, recs] = await Promise.all([
        getStudentAttendanceSummary(organization.id, selectedChild.id, sessionId),
        getStudentAttendanceHistory(organization.id, selectedChild.id, sessionId, startDate, endDate),
      ]);
      setSummary(sum);
      setRecords(recs);
    } catch (err: any) {
  ...
  useEffect(() => {
    loadAttendance();
  }, [organization, selectedChild, selectedMonth]);
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Real-time daily roll call records for{" "}
        ...
        <>
          ...
          <div className="grid gap-4 grid-cols-2 sm:grid-cols-4">
            <div className="rounded-3xl border border-border bg-card p-4 shadow-soft">
              ...
              <span className="text-[10px] text-muted-foreground font-semibold">
                of {summary?.totalDays || 0} working days
              </span>
            </div>

            <div className="rounded-3xl border border-border bg-card p-4 shadow-soft">
              <span className="text-[10px] font-bold text-muted-foreground uppercase">Absent Days</span>
            ...
            <div className="rounded-3xl border border-border bg-card p-4 shadow-soft">
              <span className="text-[10px] font-bold text-muted-foreground uppercase">Leaves</span>
              <p className="text-2xl font-black text-amber-600 mt-1">{summary?.leaveDays || 0}</p>
              <span className="text-[10px] text-muted-foreground font-semibold">Approved</span>
            </div>
            ...
            <div className="rounded-3xl border border-border bg-card p-4 shadow-soft">
              ...
              <span className="text-[10px] text-muted-foreground font-semibold">Annual Rate</span>
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-border bg-card p-6 shadow-soft space-y-4">
            <h3 className="text-sm font-extrabold text-foreground">
              Daily Attendance Logs ({selectedMonth})
            </h3>
