  ...
  const loadTimetable = async () => {
    ...
    try {
      const list = await getClassTimetable(
        organization.id,
        selectedChild.academic.classId,
        selectedChild.academic.sectionId
      );
      setEntries(list);
    } catch (err: any) {
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Weekly schedule and period allocation for Class{" "}
      ...
      {isLoading ? (
        ...
        <div className="rounded-3xl border border-border bg-card p-12 text-center shadow-soft">
          ...
          <h3 className="mt-3 text-sm font-bold text-foreground">No periods scheduled for {selectedDay}</h3>
          <p className="mt-1 text-xs text-muted-foreground">
            Check another weekday or consult the academic calendar.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
