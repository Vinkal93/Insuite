...
export const ParentHostelView: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      const [alloc, lList] = await Promise.all([
        getStudentHostelRecord(organization.id, selectedChild.id),
        listHostelLeaveRequests(organization.id, { studentId: selectedChild.id }),
      ]);
      setAllocation(alloc);
      setLeaves(lList);
      if (selectedChild.guardianPhone || selectedChild.fatherPhone || selectedChild.motherPhone) {
        setEmergencyContact(selectedChild.guardianPhone || selectedChild.fatherPhone || selectedChild.motherPhone || "");
      }
    } catch (err: any) {
      console.error("loadParentHostel error:", err);
  ...
  const handleSubmitLeave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!organization || !selectedChild || !allocation || !reason.trim() || !destination.trim() || !emergencyContact.trim()) {
      return;
    }
    ...
    try {
      ...
      alert("Out-pass leave request submitted to hostel warden.");
      setReason("");
      setDestination("");
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-black tracking-tight text-foreground sm:text-3xl">
          Hostel & Residential Info
        </h1>
        <p className="text-xs text-muted-foreground mt-0.5">
          Boarding house room and out-pass requests for{" "}
          <strong className="text-foreground">{selectedChild?.fullName}</strong>.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-3xl border border-border bg-card p-12 text-center shadow-soft">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {selectedChild?.fullName} is currently registered as a day scholar.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="rounded-3xl border border-border bg-card p-6 shadow-soft space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <h3 className="font-extrabold text-sm text-foreground">Out-Pass & Weekend Leave</h3>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Apply for weekend home visits or medical leave passes.
                </p>
              </div>
              ...
              >
                ...
                {isCreatingLeave ? "Cancel" : "Submit Leave Pass"}
              </Button>
            </div>
            ...
            {isCreatingLeave && (
              ...
              >
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div>
                    <label className="block font-semibold text-foreground mb-1">Parent Emergency Contact *</label>
                    <input
                      type="tel"
                      required
